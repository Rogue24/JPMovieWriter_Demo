...
struct File {
    ...
    struct manager {
        ...
        @discardableResult
        static func deleteFile(_ atURL: URL?, executeError: ExecuteError? = nil) -> Bool {
            guard let fileURL = atURL,
                  FileManager.default.fileExists(atPath: fileURL.absoluteString) else { return true }
            do {
        ...
        static func fileSize(_ filePath: String) -> Int {
            ...
            do {
                ...
                return fileInfo[FileAttributeKey.size] as? Int ?? 0
            } catch {
                return 0
            }
        }
    ...
    static func fileSizeString(_ fileSize: Int) -> String {
        switch fileSize {
        case (1000 * 1000)...:
            return String(format: "%.1fMB", Double(fileSize) / (1000 * 1000))
        case 1000...:
            return String(format: "%.1fKB", Double(fileSize) / 1000)
        case 0...:
            return String(format: "%zdB", fileSize)
        default:
            return "0B"
        }
    }
